...
public class BMICalculatorFX extends Application {
	...
	public BMICalculatorFX() {
		/*
		 * btnCalc.addEventHandler(arg0, arg1);(new ActionListener() {
		 * 
		 * @Override public void actionPerformed(ActionEvent arg0) { double mass; double
		 * height; try { mass = Double.parseDouble(txtMass.getText()); height =
		 * Double.parseDouble(txtHeight.getText()); } catch (NumberFormatException e) {
		 * JOptionPane.showMessageDialog(self,
		 * "Please enter a valid number for mass and height.", "Input error",
		 * JOptionPane.ERROR_MESSAGE); return; } double result = calculateBMI(mass,
		 * height); JOptionPane.showMessageDialog(self, "Your BMI is: " +
